...
|---------|-----|------|----------|
| **Default** | `26.7.3`, `latest` | Alpine | Runtime provider/SPI loading, init containers |
| **Optimized** | `26.7.3-optimized`, `optimized` | Distroless | Maximum security, near-zero CVEs, fast startup |
| **Debug** | `26.7.3-debug`, `debug` | Distroless-debug | Troubleshooting with shell |
...
docker run -d \
  ...
  -e KC_DB=dev-mem \
  ghcr.io/bysamio/keycloak:26.7.3
...
|-----|---------|-------------|
| `26.7.3` | Default | Runtime provider support, auto-build (Alpine) |
| `latest` | Default | Latest stable version with provider support |
| `26.7.3-optimized` | Optimized | Pre-built, config locked, near-zero CVEs (Distroless) |
| `optimized` | Optimized | Latest optimized version |
| `26.7.3-debug` | Debug | Debug variant with busybox shell |
...
spec:
  template:
    spec:
      ...
      containers:
        - name: keycloak
          image: ghcr.io/bysamio/keycloak:26.7.3
          env:
            - name: KC_DB
              value: postgres
            - name: KC_FEATURES
...
docker run -e KC_SPI_THEME_DEFAULT=my-theme ghcr.io/bysamio/keycloak:26.7.3
```
...
helm install keycloak oci://ghcr.io/bysamio/charts/keycloak \
  --set image.registry=ghcr.io \
  --set image.repository=bysamio/keycloak \
  --set image.tag=26.7.3-optimized \
...
```bash
# Run debug variant
docker run -it ghcr.io/bysamio/keycloak:26.7.3-debug
...
For Keycloak `26.7.3`, upstream now ships Jackson Core and Databind `2.21.5` (server libraries and flattened admin CLI alike) and pgJDBC `42.7.13`, so the Jackson and pgJDBC overlays that this image carried for `26.7.0` have been removed — keeping them would have pinned Jackson *backwards* to `2.21.4` and left duplicate classes on the classpath. Two overlays remain, both for dependencies upstream has not yet advanced: OpenTelemetry `1.62.0` over upstream `1.57.0`, keeping the `CVE-2026-45292` mitigation, and Netty `4.1.138.Final` over upstream `4.1.136.Final`, covering `CVE-2026-75595` (CRITICAL, `netty-handler`) plus `CVE-2026-75596`, `CVE-2026-49844` and `CVE-2026-59903`. All sixteen Netty modules are replaced together, including both platform classifiers of the native epoll transport, because Netty is only supported in lockstep across its modules. The Trivy ignore file suppresses `CVE-2025-59250` for `mssql-jdbc` only because the shipped artifact is the fixed `13.2.1.jre11` JAR while its embedded Maven metadata reports `13.2.1`; `26.7.3` still ships that same artifact, so the suppression stands.
